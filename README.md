...
**949 tests passing on GCC 13**, with the suite also run under ASan/UBSan with
leak detection and under ThreadSanitizer, plus four end-to-end driver scripts
that run the real binaries: one that edits and compares exported samples, one
...
maths, and one that exercises every `auscultate-cli` command.
