...
| **Momentary** | The loudest 400 ms window |
...
**Note a difference from the window.** The command-line output always names a
key, with notes attached; the window applies the refusal thresholds described in
§5.6 and will print *no key* instead. Read the strength and the contrast, not
just the name.
